...
object DBNodeSpec extends DefaultRunnableSpec {
  private val properties = List(
    ...
    testM("should update concurrently concurrently") {
      checkM(
        StorageGen.byteArray,
        StorageGen.byteArray,
        StorageGen.byteArray
        ...
        for {
          iSize <- DBNode.loadN(Int.MaxValue).map(_.length)
          _     <- DBNode.commit(A) <&> DBNode.commit(B) <&> DBNode.commit(C)
          fSize <- DBNode.loadN(Int.MaxValue).map(_.length)
        } yield {
          assert(iSize)(equalTo(fSize - 3))
        }
      })
    } @@ failure
  )
